...
```groovy
dependencies {
  implementation 'com.chronicle-labs:chronicle-sdk:0.0.11'
}
```
...
A full reference for this library is available [here](https://github.com/chronicle-labs-ai/chronicle-java/blob/HEAD/./reference.md).
...
public class Example {
    public static void main(String[] args) {
        ChronicleLabsApiClient client = ChronicleLabsApiClient
            .builder()
            .token("<token>")
            .build();
...
```java
import com.chronicle.labs.api.ChronicleLabsApiClient;
import com.chronicle.labs.api.core.Environment;
...
try{
    client.events().ingestEvent(...);
} catch (ChronicleLabsApiApiException e){
...
```java
ChronicleLabsApiHttpResponse response = client.events().withRawResponse().ingestEvent(...);
