...
public final class StreamTest {
  ...
  public void testJsonStream() {
    List<Map<String, String>> messages = Arrays.asList(createMap("message", "hello"), createMap("message", "world"));
  ...
  public void testSseStreamWithTerminator() {
    ...
    List<String> sseStrings = new ArrayList<>(events.stream().map(StreamTest::mapToSse).collect(Collectors.toList()));
  ...
  public void testSseEventDiscriminatedStream() {
    List<String> sseStrings = Arrays.asList(mapToSseWithEvent("start", createMap("status", "pending")), mapToSseWithEvent("end", createMap("status", "complete")));
    String input = String.join("\n"
            + "\n", sseStrings);
    StringReader sseInput = new StringReader(input);
